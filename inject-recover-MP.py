...
def injectplanetbeforestitch(arg):
    ...
    for s in inject_lc: 
        ...
        lc_injected = lc_injected.remove_outliers(sigma=6).normalize().remove_nans()
    ...
    period_grid = np.linspace(0.6, 11, 25000)
...
def vary_paramsmult(func, star, df,stellar_radius=1.325, trials=10000, num_processes=1):
    ...
    for s in noinjectstar:   
        ...
        lc_orig = lc_orig.remove_outliers(sigma=6).normalize().remove_nans()
        ...
        lc_orig = lc_orig[(lc_orig['flux']<1.20)&(lc_orig['flux']>0.80)].remove_nans()#.bin(time_bin_size=binsizedays)
        lc_orig['index'] = np.arange(len(lc_orig))#reindex to avoid issues with removing points
